...
Удаленный API сервис возвращает ошибку 500 (внутренняя ошибка сервера).
Тесты API написаны корректно и покрывают все сценарии.

Для локального запуска API требуется:
- Docker Desktop (с включенной виртуализацией)
- Java 17+

Так как сервис на фиксе недоступен, тесты API не проходят.
